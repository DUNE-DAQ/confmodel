...
namespace dunedaq {
ERS_DECLARE_ISSUE(confmodel,
                  InvalidOpMonFile,
                  file_name << " is an invalid name for the opmon output",
                  ((std::string)file_name))

ERS_DECLARE_ISSUE(confmodel, LoadDalFailed, "Could not load dal shared library " << library, ((std::string)library))

ERS_DECLARE_ISSUE(confmodel, ConfigurationError, , )

ERS_DECLARE_ISSUE_BASE(confmodel,
...
ERS_DECLARE_ISSUE_BASE(confmodel,
                       ...
                       ConfigurationError,
                       "The control service has been defined multiple times for the application " + app_name +
                         " you need to define only one service called control",
                       ,
                       ((std::string)app_name)

)

ERS_DECLARE_ISSUE_BASE(confmodel, BadConf, ConfigurationError, what << where, , ((std::string)what)((std::string)where))

ERS_DECLARE_ISSUE_BASE(confmodel,
                       MissingConstructor,
                       ConfigurationError,
                       "Configuration contains a " + base + " derived object " + obj + " which cannot be cast to " +
                         base + " due to the constructor for the class not being available in this application",
                       ,
                       ((std::string)base)((std::string)obj))

ERS_DECLARE_ISSUE(confmodel, AlgorithmError, , )

ERS_DECLARE_ISSUE_BASE(confmodel, BadVariableUsage, AlgorithmError, message, , ((std::string)message))

ERS_DECLARE_ISSUE_BASE(confmodel,
                       BadApplicationInfo,
                       AlgorithmError,
                       "Failed to retrieve information for Application \'" << app_id
...
ERS_DECLARE_ISSUE_BASE(confmodel,
                       BadProgramInfo,
                       AlgorithmError,
                       "Failed to retrieve information for Program \'" << prog_id
                                                                       << "\' from the database: " << message,
                       ,
...
ERS_DECLARE_ISSUE_BASE(confmodel,
                       ...
                       AlgorithmError,
                       "The " << obj_id << '@' << obj_class << " object " << address
                              << " was not initialized by DAL algorithm " << algo,
                       ,
                       ((std::string)obj_id)((std::string)obj_class)((void*)address)((const char*)algo))
...
ERS_DECLARE_ISSUE_BASE(confmodel,
                       ...
                       "Reach maximum allowed recursion ("
                         ...
                         << "; possibly there is circular dependency between these objects: " << objects,
                       ,
...
ERS_DECLARE_ISSUE_BASE(
  ...
  NoJarFile,
  AlgorithmError,
  "Cannot find jar file \'" << file << "\' described by \'" << obj_id << '@' << obj_class << "\' that is part of \'"
                            << rep_id << '@' << rep_class << '\'',
  ,
  ((std::string)file)((std::string)obj_id)((std::string)obj_class)((std::string)rep_id)((std::string)rep_class))
...
ERS_DECLARE_ISSUE_BASE(confmodel,
                       ...
                       AlgorithmError,
                       "Has exceeded the maximum of iterations allowed (" << limit
                                                                          << ") during calculation of excluded objects",
                       ,
                       ((unsigned int)limit))
