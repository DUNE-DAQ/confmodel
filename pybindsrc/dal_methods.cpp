...
session_get_all_applications(Configuration& db, const std::string& session_name)
{
...
std::vector<ObjectLocator>
session_get_included_applications(Configuration& db, const std::string& session_name)
{
  auto session = db.get<Session>(session_name);
  std::vector<ObjectLocator> apps;
...
void
exclude_entity(Configuration& db, const std::string& session_id, const std::string& component_id)
{
...
void
include_entity(Configuration& db, const std::string& session_id, const std::string& component_id)
{
  auto session_ptr = const_cast<dunedaq::confmodel::Session*>(db.get<dunedaq::confmodel::Session>(session_id));
  auto component_ptr = db.get<dunedaq::confmodel::ExcludableEntity>(component_id);
  if (session_ptr == nullptr) {
...
bool
entity_excluded(Configuration& db, const std::string& session_id, const std::string& component_id)
{
  const dunedaq::confmodel::Session* session_ptr = db.get<dunedaq::confmodel::Session>(session_id);
  const dunedaq::confmodel::ExcludableEntity* component_ptr =
    db.get<dunedaq::confmodel::ExcludableEntity>(component_id);
  if (component_ptr == nullptr) {
    return false;
  }
...
std::vector<std::vector<ObjectLocator>>
entity_get_parents(Configuration& db, const std::string& session_id, const std::string& component_id)
{
  const dunedaq::confmodel::Session* session_ptr = db.get<dunedaq::confmodel::Session>(session_id);
  const dunedaq::confmodel::ExcludableEntity* component_ptr =
    db.get<dunedaq::confmodel::ExcludableEntity>(component_id);
  ...
  for (const auto& parent : parents) {
    ...
    for (const auto& ancestor_component_ptr : parent) {
      parents_components.emplace_back(
        ObjectLocator(ancestor_component_ptr->UID(), ancestor_component_ptr->class_name()));
    }
...
daq_application_get_used_host_components(Configuration& db, const std::string& app_id)
{
...
std::vector<std::string>
daq_application_construct_commandline_parameters(Configuration& db,
...
std::vector<std::string>
rc_application_construct_commandline_parameters(Configuration& db,
                                                const std::string& session_id,
                                                const std::string& app_id)
{
...
std::string
d2d_receiver(Configuration& db, const std::string& d2d_id)
{
...
std::vector<std::string>
d2d_senders(Configuration& db, const std::string& d2d_id)
{
...
std::vector<std::string>
d2d_streams(Configuration& db, const std::string& d2d_id)
{
...
std::vector<std::string>
exclude_entity_set_contains(Configuration& db, const std::string& res_id)
{
  std::vector<std::string> resources;
...
register_dal_methods(py::module& m)
{
  ...
  m.def("entity_excluded",
        &entity_excluded,
        "Determine if a ExcludableEntity-derived object (e.g. a Segment) has been excluded");
  m.def("entity_get_parents",
        &entity_get_parents,
        "Get the ExcludableEntity-derived class instances of the parent(s) of the ExcludableEntity-derived object in "
        "question");
  m.def("daqapp_get_used_host_components",
        &daq_application_get_used_host_components,
        "Get list of HostExcludableEntitys used by DAQApplication");
  m.def("daq_application_construct_commandline_parameters",
        &daq_application_construct_commandline_parameters,
        "Get a version of the command line agruments parsed");
  m.def("rc_application_construct_commandline_parameters",
  ...
  m.def("exclude_entity_set_contains",
        &exclude_entity_set_contains,
